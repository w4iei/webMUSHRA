...
FinishPage.prototype.sendResults = function() {
  var errorMessage = this.dataSender.send(this.session);
  if (errorMessage) {
    this.errorDiv.text(errorMessage);
    if (!$.contains($("#popupResultsContent")[0], this.errorDiv[0])) {
      $("#popupResultsContent").prepend(this.errorDiv);
    }
  }
...
FinishPage.prototype.render = function (_parent) {
    ...
    if (this.pageConfig.showResults) {
      ...
      for (i = 0; i < trials.length; ++i) {
        ...
        } else if (trial.type === "paired_distance") {
          trPairedDistance = document.createElement("tr");
          thT = $("<th colspan='2'></th>");
          $(thT).append(trial.id + " (Paired Distance)");
          $(trPairedDistance).append(thT);
          $(table).append(trPairedDistance);

          var j;
          for (j = 0; j < trial.responses.length; ++j) {
            trPD = document.createElement("tr");
            tdPDLabel = document.createElement("td");
            tdPDValue = document.createElement("td");

            var response = trial.responses[j];

            $(tdPDLabel).text("Distance");
            $(tdPDValue).text(response.distance);
            $(trPD).append(tdPDLabel);
            $(trPD).append(tdPDValue);
            $(table).append(trPD);

            trPDComment = document.createElement("tr");
            tdPDCommentLabel = document.createElement("td");
            tdPDCommentValue = document.createElement("td");

            $(tdPDCommentLabel).text("Comment");
            $(tdPDCommentValue).text(response.comment || "");
            $(trPDComment).append(tdPDCommentLabel);
            $(trPDComment).append(tdPDCommentValue);
            $(table).append(trPDComment);
          }
          trEmpty = $("<tr height='8px'></tr>");
          $(table).append(trEmpty);
