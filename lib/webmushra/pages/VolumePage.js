...
VolumePage.prototype.render = function (_parent) {
  var volumeContent = $("<div style='width: 100%; min-height: 12em; overflow: hidden; background: white;'></div>");
  var content = $(" <p> " + this.pageConfig.content + " </p>");
  var slider = $("<table style='width: 100%; max-width: 600px; margin: 0 auto;'></table>");
  slider.append(
    $('<tr></tr>').append(
      $('<td></td>').append(
  ...
  volumeContent.append(content);
  this.filePlayer.render(volumeContent);
  volumeContent.append(slider);
  _parent.append(volumeContent);
};
