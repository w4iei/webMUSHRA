...
function PairedDistancePage(_pageManager, _pageTemplateRenderer, _audioContext, _bufferSize, _audioFileLoader, _session, _pageConfig, _errorHandler, _language) {
  ...
  this.suppressCommentFocus = false;
}
...
PairedDistancePage.prototype.render = function(_parent) {
  ...
  radioContainer.find("input[type='radio']").bind("change", (function() {
    ...
    if (!this.suppressCommentFocus) {
      $('#paired-distance-comment').focus();
    }
  }).bind(this));
  div.append(radioContainer);

  div.append($("<p><strong>" + this.pageManager.getLocalizer().getFragment(this.language, 'pairedDistanceCommentLabel') + "</strong></p>"));
  div.append($(
    "<textarea id='paired-distance-comment' rows='4' placeholder='" +
    this.pageManager.getLocalizer().getFragment(this.language, 'pairedDistanceCommentPlaceholder') +
    "'></textarea>"
  ));
  $('#paired-distance-comment').bind('keydown', (function(event) {
    if ((event.metaKey || event.ctrlKey) && (event.key === 'Enter' || event.keyCode === 13)) {
      event.preventDefault();
      return this.advanceToNextPage();
    }
  }).bind(this));
  div.append($("<p><small>" + this.pageManager.getLocalizer().getFragment(this.language, 'pairedDistanceCommentShortcutHelp') + "</small></p>"));
...
PairedDistancePage.prototype.playStimulus = function(_index) {
  ...
  this.genericAudioControl.audioCurrentPositions[_index] = 0;
...
PairedDistancePage.prototype.selectDistance = function(_value, _suppressCommentFocus) {
  this.suppressCommentFocus = _suppressCommentFocus === true;
...
PairedDistancePage.prototype.isCommentFieldFocused = function() {
  return document.activeElement && document.activeElement.id === 'paired-distance-comment';
};

PairedDistancePage.prototype.advanceToNextPage = function() {
...
PairedDistancePage.prototype.bindShortcuts = function() {
  Mousetrap.bind(['q'], (function() {
    if (this.isCommentFieldFocused()) {
      return true;
    }
    $('#pairedDistancePlay1').click();
    return false;
  }).bind(this));
  ...
  Mousetrap.bind(['e'], (function() {
    ...
    $('#pairedDistancePlaySequence').click();
    return false;
  }).bind(this));
...
PairedDistancePage.prototype.load = function() {
  ...
  if (this.distance === null) {
    ...
    this.selectDistance(this.distance, true);
  }
  $('#paired-distance-comment').val(this.comment);
};
...
PairedDistancePage.prototype.save = function() {
  ...
  this.comment = $('#paired-distance-comment').val();
  this.genericAudioControl.freeAudio();
};

PairedDistancePage.prototype.store = function() {
  if (this.pageConfig.storeResults === false) {
    return;
  }
  ...
  rating.comment = $.trim(this.comment);
